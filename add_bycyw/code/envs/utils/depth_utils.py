...
def get_camera_matrix(width, height, fov):
    ...
    arr = np.asarray([[f, xc, 0], [0, 1, 0], [0, zc, f]])

    camera_matrix = {'xc': xc, 'zc': zc, 'f': f, 'array': arr}
    camera_matrix = Namespace(**camera_matrix)

    return camera_matrix


def projectPoints(pt3D, Rmat, tvec, camera_matrix, img_height):
    A = np.concatenate((Rmat, tvec[:, np.newaxis]), axis=-1)
    pt4D = np.concatenate((pt3D, [1]))
    homogeneous = camera_matrix @ A @ pt4D
    x = homogeneous[0] / homogeneous[1]
    z = homogeneous[2] / homogeneous[1]

    # switch the z axis, since +z direction in 3D space is up, while +y direction in image space is down
    z = img_height - z
...
def transform_camera_view_t_multiple(XYZ, sensor_height, camera_elevation_degrees, device):
    ...
    block_result = torch.matmul(blockXYZ, blockR)

    to_concat = []
    for i in range(num_procs):
        to_concat.append(block_result[22500*i:22500*(i+1), 3*i:3*(i+1)])
