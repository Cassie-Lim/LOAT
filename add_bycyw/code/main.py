...
# ****************
import add_bycyw.code.envs.utils.pose as pu
from add_bycyw.code.envs import make_vec_envs
import json
# *************
...
'''
NOTE 这个版本代码已经加上用mask来分离拿着的物体，如果需要使用原本的版本，需要注释代码

TODO 1. 打开冰箱门失败后，退一步再打开；2. 测试一下put object 不centering的效果：不行，很有可能就是，它刚看到要放的物体，还离得挺远的，然后就直接放了，结果就失败，不过，至起码，放在moveable容器里面是不用centering的，因为拿的东西很小，应该也不存在放不进去的现象？
'''
