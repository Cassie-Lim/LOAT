...
# *****************
from add_bycyw.code.ALFRED_task_helper import get_list_of_highlevel_actions, determine_consecutive_interx, get_arguments, get_arguments_test, read_test_dict,get_list_of_highlevel_actions_from_plan
# *******************
...
class Sem_Exp_Env_Agent_Thor(ThorEnvCode):
    ...
    def __init__(self, args, scene_names, rank):
        ...
        # super().__init__(args, rank, player_screen_height=600, player_screen_width=600)
    ...
    def load_initial_scene(self):
        ...
        try:
            ...
            # # NOTE 为了测试是不是因为拿起箱子，使得建图很奇怪，这里将pickup box去掉，跑代码时一定要取消掉这里
            # list_of_actions = [action_tuple for action_tuple in list_of_actions if action_tuple!= ('Box', 'PickupObject')]
            # caution_pointers = [1,2]
    ...
    def is_visible_from_mask(self, mask, visibility_threshold=1):
        ...
        gnd_dist = np.percentile(dists, 25) #从小到大的第25个分位点，TODO 放东西的时候，这几百分数缩短？
    ...
    def consecutive_interaction(self, interaction, interaction_mask):
        ...
        if interaction == "PickupObject":
            ...
            if success:
                ...
                # if self.goal_name == "Box":
                #     # try:
                #     #     self.step(dict(action="MoveHeldObjectUp",
                #     #             moveMagnitude=0.5,
                #     #             forceVisible=False)
                #     #         )
                #     # except:
                #     #     print("MoveHeldObjectUp fail")
                #     # try:
                #     #     self.step(dict(                    
                ...
                #     #         forceVisible=False))
                #     # except:
                #     #     print("MoveHeldObject fail")
                #     # 可能ai2thor版本问题，这两个动作都无法执行
                #     try: 
                #         self.step(dict(action="ThrowObject",
                #             moveMagnitude=150.0,
                #             forceAction=True))
                #     except:
                #         print("ThrowObject fail")
                #     try: 
                #         self.step(dict(action="ThrowObject",
                #             moveMagnitude=100.0,
                #             forceAction=True))
                ...
                #     try:
                #         self.step(
                #             dict(action="DropHandObject",
                #             forceAction=True)
                #         )
                #     except:
                #         print("DropHandObject fail")
                # # 箱子一扔就挡住机器人
                # # ******************
    ...
    def get_traversible_new(self, grid, planning_window):
        ...
        # # ***********************
        # # 在用gt depth的时候，减少膨胀的尺寸
        # if self.args.use_learned_depth:
        #     obstacles = skimage.morphology.binary_dilation(obstacles, self.selem)
        # else:
        #     selem = skimage.morphology.square(1)
        #     obstacles = skimage.morphology.binary_dilation(obstacles, selem)
        #     # cv2.imwrite(f'debug_image/obstacles_{self.steps_taken}_{0}.png',obstacles*255)
        #     # for i in range(4,0,-1):
        #     #     selem = skimage.morphology.square(i)
        #     #     # self.selem = skimage.morphology.square(self.args.obstacle_selem)
        #     #     # 默认self.args.obstacle_selem 是9，这里将它减小
        #     #     obstacles_new = skimage.morphology.binary_dilation(obstacles, selem)
        #     #     obstacles_new[collision_map] = 1
        #     #     obstacles_new[self.visited[gy1:gy2, gx1:gx2][y1:y2, x1:x2] == 1] = 0
        #     #     traversible = np.logical_not(obstacles_new)
        #     #     if not self.isTrapped(traversible, planning_window):
        #     #         print(f"agent is not trapped, when the selem is {i}")
        #     #         obstacles = obstacles_new
        #     #     cv2.imwrite(f'debug_image/obstacles_{self.steps_taken}_{i}.png',obstacles_new*255)
        #     #     # 为什么这里数字越小，还膨胀得越厉害？因为是for循环，每一步都是上一步得基础上，继续做,代码已修改
        # 问题不在这里，不是因为膨胀使得机器人trapped，而是因为地毯有一定高度，把地毯也认为是障碍物了
        # 只膨胀1的话，机器人容易贴着障碍物走，还是会撞到（侧面说明这个建图过程误差还挺大的？也不是，需要考虑机器人的大小）
        # # *******************
        ...
        # NOTE 这里不对吧，collision map只有发生碰撞才会有，这种情况下，会导致很多地方都可traversible，结果就经常碰撞
        if self.isTrapped(traversible, planning_window):#与当前位置连通太少时，就会认为trapped?
    ...
    def getNextAction(self, planner_inputs, during_lookaround, target_offset, target_xyz, force_slice_pickup):
        ...
        # *****************
            ...
            # **************
            print(f"goal_coord is {goal_coord},and target_offset is {target_offset}")
            # ***********
    ...
    def plan_act_and_preprocess(self, planner_inputs, goal_spotted):
        ...
        print(f"current steps is {self.steps}")
        if self.steps == 10:
            print("debug")
        # ****************
        ...
        if planner_inputs["wait"]:
            ...
            return np.zeros(self.obs.shape), 0., False, self.info, False, {
                ...
                'actseq':{(self.args.from_idx + self.scene_pointer* self.args.num_processes + self.rank, self.traj_data['task_id']): self.actions[:1000]},
                'errs': self.errs, 'logs':self.logs, 'current_goal_sliced':self.cur_goal_sliced,
                'next_goal': next_goal, 'delete_lamp': False, 'fails_cur': 0},planner_inputs['list_of_actions']
            # *******************
        ...
        if planner_inputs["manual_step"] is not None:
            ...
            print("manul step")
            # 调用一下plan代码，调试建图用
            during_lookaround = False
            target_offset = 0
            goal_xyz = None
            force_slice_pickup = False
            ...
            obs, rew, done, info, goal_success, next_step_dict = self.manualControl(planner_inputs["manual_step"])
            return obs, rew, done, info, goal_success, next_step_dict, planner_inputs['list_of_actions']
        ...
        # # ****************
        # # NOTE 为了调试，将这里条件修改了
        # interact_ok = (not self.force_move) and ((not needs_centering) or (goal_spotted) or self.reached_goal)
        # # 这样改也不行，centering就没用了
        # # **************
        # *************add by me **************
        if goal_spotted:
            print("found goal")
    ...
    def _get_stg(self, grid, start, goal_coord, planning_window, target_offset, measure_offset_from_edge=False):
        ...
        # 这里的new_goal是下一步要去的位置，仅仅用来可视化

        if self.args.save_pictures:
            viz = np.repeat(255 - traversible[:, :, np.newaxis] * 255, 3, axis=2)
            # traversible可以走的地方是1，因此viz里面可以走的地方是白色，不可以走的地方是黑色
            h, w = viz.shape[:2]
            viz[max(0, goal_coord[0]-2):min(h-1, goal_coord[0]+3), max(0, goal_coord[1]-2):min(w-1, goal_coord[1]+3), :] = [255, 0, 0]
            viz[max(0, new_goal[0]-3):min(h-1, new_goal[0]+4), max(0, new_goal[1]-2):min(w-1, new_goal[1]+3), :] = [0, 0, 255]
            # ***********
            # 添加当前位置的标记
    ...
    def _preprocess_obs(self, obs):
        ...
        depth = self._preprocess_depth_new(depth,sem_seg_pred)
    ...
    # *************************
    def _preprocess_depth_new(self, depth,sem_seg_pred,cal_optical_flow=False):
        depth = depth[:, :, 0] * 100

        if self.picked_up:
            goal_idx = self.total_cat2idx[self.pick_up_obj]
            self.picked_up_mask = sem_seg_pred[:,:,goal_idx]
            self.picked_up_mask = skimage.morphology.dilation(self.picked_up_mask,skimage.morphology.square(10))
            # self.picked_up_mask = skimage.morphology.dilation(self.picked_up_mask,skimage.morphology.square(14)) #提高之后那个场景还是有噪点
            if cal_optical_flow:
                # 计算光流
            ...
            depth[np.where(self.picked_up_mask)] = 10000.0

            # # mask_err_below = depth < 25 #有时候mask去的不准确,用这个也不准确，先这样吧
            # print(f"{len(np.where(mask_err_below)[0])}")
            # cv2.imwrite(f"debug_image/image/mask_err_below_{self.steps_taken}_{len(np.where(mask_err_below)[0])}.png",mask_err_below*255)
            # depth[np.logical_or(self.picked_up_mask, mask_err_below)] = 10000.0
