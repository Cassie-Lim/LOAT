...
class ThorEnvCode(Controller):
    ...
    def __init__(self, args, rank, x_display=constants.X_DISPLAY,
        ...
        super().__init__(quality='Very High')
    ...
    def setup_scene(self, traj_data, task_type, r_idx, args, reward_type='dense'):
        ...
        if collect_data:
            for obj in objects:
                tmp_dict = obj.copy()
                tmp_dict["scene"] = self.event.metadata["sceneName"]
                tmp_dict["short_name"] = tmp_dict["name"].split('_')[0]
                tmp_dict["short_scene"] = tmp_dict["scene"].split('_')[0]
                tmp_dict["episode_number"] = self.room_data_counter
                self.room_data.append(tmp_dict)

            df = pd.DataFrame(self.room_data)
            df.to_csv("alfred_runinfo.csv")
            self.room_data_counter += 1
    ...
    def reset(self, scene_name_or_num,
        ...
        self.scene_num = scene_name_or_num
    ...
    def approx_success(self, do_log=False):
        diff_thresh = 2
        diff_map = cv2.absdiff(self.prev_rgb, self.event.frame)
        diff_mask = np.any(diff_map > diff_thresh, axis=2)

        h, w = diff_mask.shape[:2]
        area = np.count_nonzero(diff_mask) / (h * w)
        area_thresh = 0.01
        success = area > area_thresh

        if do_log:
            self.print_log(f"action success:{success}, area changed:{area}")
    ...
    def step(self, action, smooth_nav=False):
        ...
        if action['action'] == '<<stop>>':
            ...
            if smooth_nav:
                ...
                if "LookUp" in action['action']:
                    ...
                    elif self.args.approx_last_action_success and self.approx_success():
                        self.camera_horizon += -angle
        ...
        if not(self.args.approx_pose):
            ...
            if self.args.approx_last_action_success:
                whether_success = self.approx_success(do_log=True)
            else:
    ...
    def get_current_image(self):
        return self.event.frame.copy()  # shape (h, w, 3)
    ...
    def va_interact(self, action, interact_mask=None, smooth_nav=False, mask_px_sample=1, debug=False):
        ...
        if type(interact_mask) is str and interact_mask == "NULL":
            ...
            instance_segs = cv2.resize(instance_segs, (300, 300))
            ...
            if len(instance_ids) == 0:
                if not ("Rotate" in action or "MoveAhead" in action or "Look" in action):
                    ...
                    # delete "sensor_pose", since the agent never moved
                    self.info["sensor_pose"] = [0, 0, 0]
        ...
        try:
            ...
            # # ********************
            # if target_instance_id == '' and action == 'PickupObject':
            #     obs, rew, done, infos, event, api_action = self.to_thor_api_exec('LookUp_0', target_instance_id, smooth_nav)
            # else:
            #     obs, rew, done, infos, event, api_action = self.to_thor_api_exec(action, target_instance_id, smooth_nav)
            # # *********************
            # # TODO 之后要把这部分还原回来
