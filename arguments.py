...
def get_args():
    ...
    parser.add_argument('--camera_height', type=float, default=1.576,
    ...
    parser.add_argument('--collision_obstacle_length', type=int, default=9)
    ...
    parser.add_argument('--splits', type=str,
                        default="alfred_data_small/splits/oct21.json")
    parser.add_argument('--data', type=str,
                        default="alfred_data_small/json_2.1.0")
    parser.add_argument('--reward_config', default='models/rewards.json')
    parser.add_argument('--eval_split', type=str, default='valid_seen', choices=[
                        'train', 'valid_seen', 'valid_unseen', 'tests_unseen', 'tests_seen'])
    parser.add_argument('--model_path', type=str, default="model.pth")
    parser.add_argument('--model', type=str,
                        default='models.model.seq2seq_im_mask')
    ...
    parser.add_argument('--subgoals', type=str,
                        help="subgoals to evaluate independently, eg:all or GotoLocation,PickupObject...", default="")
    ...
    parser.add_argument('--skip_model_unroll_with_expert',
                        action='store_true', help='forward model with expert actions')
    parser.add_argument('--no_teacher_force_unroll_with_expert',
                        action='store_true', help='no teacher forcing with expert')

    # debug
    parser.add_argument(
        '--debug_alfred', dest='debug_alfred', action='store_true')
    parser.add_argument('--fast_epoch', dest='fast_epoch', action='store_true')

    parser.add_argument(
        '--gts', dest='ground_truth_segmentation', action='store_true')
    parser.add_argument('--learned_depth',
                        dest='use_learned_depth', action='store_true')
    parser.add_argument('--valts_depth', dest='valts_depth',
                        action='store_true')
    parser.add_argument('--depth_checkpoint_path', type=str,
                        default="real_test_ogn/lr1e-5/model-48000-best_d1_0.88987")

    parser.add_argument('--focal', type=float)  # one of 518 or 259
    parser.add_argument('--depth_gpu', type=int, default=0)

    parser.add_argument(
        '--ignore_cats', dest='ignore_categories', action='store_true')
    parser.add_argument('--wait_time', type=float, default=0)

    parser.add_argument('--scene', type=str, default='vase')

    parser.add_argument('--look_up_down', type=str,
                        default="Down")  # 'Up' or 'Down'
    parser.add_argument('--view_angle', type=float,
                        default=0)  # look up or down angle
    # decrease or increase agent height
    parser.add_argument('--agent_height_change', type=float, default=0)

    parser.add_argument('--top_thirty', dest='top_thirty', action='store_true')
    parser.add_argument('--no_common_objs',
                        dest='no_common_objs', action='store_true')
    ...
    parser.add_argument('--x_display', type=str)
    ...
    parser.add_argument('--leaderboard', dest='leaderboard',
                        action='store_true')
    parser.add_argument('--debug_max_steps',
                        dest='debug_max_steps', action='store_true')
    parser.add_argument('--tomato', dest='tomato', action='store_true')

    parser.add_argument(
        '--no_look_down', dest='no_look_down', action='store_true')
    parser.add_argument('--apple', dest='apple', action='store_true')

    parser.add_argument('--debug_local', dest='debug_local',
                        action='store_true')
    parser.add_argument('--save_pictures',
                        dest='save_pictures', action='store_true')
    parser.add_argument('--save_training_pictures',
                        dest='save_training_pictures', action='store_true')

    parser.add_argument('--exclude_list',  type=str, default="")
    parser.add_argument('--load_cuda_goals',
                        dest='load_cuda_goals', action='store_true')
    parser.add_argument('--save_cuda_goals',
                        dest='save_cuda_goals', action='store_true')
    ...
    parser.add_argument(
        '--delete_from_map_after_move_until_visible', action='store_true')
    ...
    parser.add_argument('--obstacle_selem', type=int, default=9)

    parser.add_argument('--valts_trustworthy', action='store_true')
    parser.add_argument('--valts_trustworthy_prop', type=float, default=1.0)
    parser.add_argument('--valts_trustworthy_obj_prop',
                        type=float, default=1.0)
    parser.add_argument('--valts_trustworthy_obj_prop0',
                        type=float, default=1.0)
    ...
    parser.add_argument('--language_granularity', type=str,
                        choices=["high", "high_low", "gt"], default="high")
    ...
    parser.add_argument('--sem_policy_type', type=str,
                        choices=["none", "mlm", "cnn","seq"], default="none")
    parser.add_argument('--grid_sz', type=int, default=8)
    parser.add_argument('--mlm_fname', type=str,
                        default="mlmscore_bert_base_cased_mlmSingleQ")
    parser.add_argument('--mlm_options', type=str, nargs='+', default=[])
    parser.add_argument('--mlm_temperature', type=float, default=1.0)
    ...
    parser.add_argument('--target_offset_interaction', type=float, default=0.5,
                        help='amount to shift the goal when interacting with the target')
    parser.add_argument('--ignore_sliced', action='store_true', default=False,
                        help='remove "Sliced" from the language interpretation, since the visual recognition modules cannot differentiate between sliced and the original')

    parser.add_argument('--manual_control', action='store_true', default=False,
                        help='manually control the agent')

    parser.add_argument('--debug_env', action='store_true', default=False,
                        help='use debugging environment (can breakpoint, etc.)')

    parser.add_argument('--sem_search_del_size', type=int, default=11,
                        help='size to remove from semantic search prediction after failed search')
    parser.add_argument('--goal_search_del_size', type=int, default=11,
                        help='size to remove from observation map after failed search')

    parser.add_argument('--centering_strategy', type=str,
                        choices=["local_adjustment", "simple", "none"],
                        default="local_adjustment")

        # ********************
    parser.add_argument('--run_idx_file', type=str, default=None)
    # 要运行的id的文件
    parser.add_argument('--subgoal_file', type=str, default=None)
    # 要使用的subgoal文件，注意，这个文件需要以存储为json列表，每个item是一个字典，分别为id,high,plan
    # 由于多线程的原因，每次都是先加载完所有文件，然后分配给各个线程
    # 保存exo图
    parser.add_argument('--save_exp', action='store_true', default=False)
    parser.add_argument('--save_exp_dir', type=str, default=f"/raid/cyw/Prompter/log/explored_area/")
    # 长时间找不到目标的时候，更换机器人仰角
    parser.add_argument('--change_altitude', action='store_true', default=False)
    # 是否要使用新训练好的mask_RCNN
    parser.add_argument('--use_new_mask_rcnn', action='store_true', default=False)
    # 新mask_rcnn的模型路径
    parser.add_argument('--mask_rcnn_path', type=str, default=None)
    # expored_area缩小的默认大小，默认为11，这个参数只有在--change_altitude有效时使用
    parser.add_argument('--explored_selem', type=int, default=3)
    # 允许请求next goal的最大次数
    parser.add_argument('--max_next_goal_request', type=int, default=8)
    # 在使用change_altitude时，该参数表示在当前俯仰角下，请求下一个目标的个数，当请求个数超过该值时，replan；在不使用change_altitude时，在所有俯仰角下，请求next goal个数超过该数就请求replan
    # 是否使用replan，该参数只有在change_altitude有效的时候才有效
    parser.add_argument('--use_replan', action='store_true', default=False)
    # 是否记录一些replan，subgoal_list的文件
    parser.add_argument('--record_replan', action='store_true', default=False)
    # 结果保存的文件
    ...
    parser.add_argument("--drop_interaction_fail_loc",action='store_true', default=False)
    # 去除掉交互失败的位置，交互的目标点，不选取这些位置
    # ************
        ...
        if args.auto_gpu_config:
            ...
            for i in range(num_gpus):
                gpu_memory = min(gpu_memory,
                                 ...
                                 / 1024/1024/1024)
                if i == 0:
                    assert gpu_memory > min_gpu_memory, "Insufficient GPU memory"

            num_processes_per_gpu = int(gpu_memory/2.6)
            num_processes_on_first_gpu = int((gpu_memory - min_gpu_memory)/2.6)
