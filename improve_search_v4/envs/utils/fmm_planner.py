...
def planNextMove(traversible, step_size, start, goal, target_offset, measure_offset_from_edge,goal_drop_loc=None):
    '''
    新增加了一个参数：goal_drop_loc，shift goal的时候，不shift到这些地方
    '''
    ...
    goal_fixed = (len(goal) == 3) #goal由sem_exp_thor传进来，什么时候才能是goal_fixed呢？使用sliced_pose作为goal的时候
    next_goal = False
    if goal_fixed:
        goal_free_and_not_shifted = False # goal fix就一定会shift goal 吗
        ...
        # 只有开东西才会shift,这里先不处理，之后看结果 TODO 再决定是否要去掉
...
#     goal_fixed = (len(goal) == 3) #goal由sem_exp_thor传进来，什么时候才能是goal_fixed呢？
#     next_goal = False
#     if goal_fixed:
#         goal_free_and_not_shifted = False # goal fix就一定会shift goal 吗
#         arrived_at_goal = (start_pose == goal_pose)
