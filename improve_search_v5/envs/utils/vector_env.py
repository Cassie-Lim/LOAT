...
class VectorEnv:
    ...
    def _worker_env(
        ...
        try:
            ...
            while command != CLOSE_COMMAND:
                ...
                # 该方法没有返回值，不需要加connection_write_fn
                # *************************
                ...
                # ******************
                    ...
                    obs, rew, done, info, success = env.consecutive_interaction(
                        data[0], data[1])
    ...
    def va_interact(self, action, interact_mask, smooth_nav, mask_px_sample, debug):
        ...
        for e, write_fn in enumerate(self._connection_write_fns):
            if len(self._connection_write_fns) == 1:
                data_list = [action, interact_mask,
                             smooth_nav, mask_px_sample, debug]
            else:
                data_list = [action[e], interact_mask[e],
                             smooth_nav[e], mask_px_sample[e], debug[e]]
            write_fn((VA_INTERACT_COMMAND, data_list))
        results = []
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        obs, rew, done, infos, success, event, target_instance_id, emp, api_action = zip(
            *results)
        self._is_waiting = False
