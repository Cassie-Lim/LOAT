...
def determine_consecutive_interx(list_of_actions, previous_pointer, sliced=False):
    ...
    if previous_pointer <= len(list_of_actions)-1:
        ...
        elif list_of_actions[previous_pointer][1] == "OpenObject" and (list_of_actions[previous_pointer+1][1] == "PickupObject" or list_of_actions[previous_pointer+1][1] == "PutObject"):
            # *******加上开冰箱并且放东西的时候也consecutive
            returned = True
    ...
    if type_set == 'replace_target':
        ...
        for i in range(len(list_of_actions)):
            ...
            if i == index:
                ...
                # 开东西找了，caution_pointers应该去掉
                if i in caution_pointers:
                    caution_pointers.remove(i)
                if second and i<len(second_object):
        ...
        for i in range(index,len(list_of_actions)-1):
            if (i+2>=len(list_of_actions) or list_of_actions[i+2][0]!= to_search_target) and list_of_actions[i+1][1]=='CloseObject' and list_of_actions[i+1][0]!= to_search_target:
                close_index = i+1 #报错close_index没有指定:开了东西，没有关上
                break
        if not close_index == -1:
            ...
            list_of_actions_new, second_object_new, caution_pointers_new = list_of_actions, second_object, caution_pointers
    else:
