...
'''
如果要取消可视化图像，搜索“self.args.save_pictures”，注释掉相应的段即可
'''
...
class SemgnetationHelper:
	...
	def __init__(self, agent):
		...
		with open(self.route, 'w') as f:
			writer = csv.writer(f)
			writer.writerow(['Scene_Num', 'Step_NO', 'GT_found', 'YOLO_found', 'goal', 'imgTag', 'size'])
		...
		if args.use_sem_seg:
			...
			self.sem_seg_model_alfw_large.to(self.sem_seg_gpu)                  
	...
	def compare_GT(self, yolo_result):
		
		gt_objs = []
		gt_have = False
		yolo_have = False
		gt_size = 0
		if 'Sliced' in self.cp_goal:
			self.cp_goal = self.cp_goal.split('Sliced')[0]
			# print(self.cp_goal)

		for obj_name in self.agent.last_event.instance_detections2D:
			obj_id = obj_name.split('|')[0]
			if "Basin" in obj_name:
				obj_id += "Basin"
			gt_objs.append(obj_id)
		if self.cp_goal not in gt_objs:
			# return
			gt_have = False
		else:
			for obj_name in self.agent.last_event.instance_detections2D:
				if self.cp_goal in obj_name:
					xyxy = self.agent.last_event.instance_detections2D[obj_name]
					gt_size = abs(xyxy[0]-xyxy[2])*abs(xyxy[1]-xyxy[3])
			gt_have = True
		for i in range(len(yolo_result.boxes.cls)):
			cls_id = yolo_result.boxes.cls[i]
			name = yolo_result.names[int(cls_id)]
			if name == self.cp_goal:
				yolo_have = True
				break
		if not gt_have and not yolo_have:
			return
		with open(self.route, 'a') as f:
			writer = csv.writer(f)
			if gt_have and yolo_have:
				writer.writerow([self.agent.scene_num ,self.agent.steps_taken, gt_have, yolo_have, self.cp_goal,'',gt_size])
				return
			else:
				writer.writerow([self.agent.scene_num ,self.agent.steps_taken, gt_have, yolo_have, self.cp_goal, self.img_tag,gt_size])
		
		seg_img = yolo_result.plot()
		imgs_path = './failed_seen'
		cv2.imwrite(os.path.join(imgs_path, f"{self.agent.scene_num}_yolo_seg-yolo_have_{yolo_have}-gt_have_{gt_have}-goal_{self.cp_goal}-step_{self.agent.steps_taken}_{self.img_tag}.png"), seg_img)
		self.img_tag += 1
		# imgs_name = os.

	def collect_wrong(self, yolo_result):
		gt_objs = []
		yolo_objs = []
		gt_have = True
		yolo_have = True
		...
		for obj_name in self.agent.last_event.instance_detections2D:
			...
			if "Basin" in obj_name:
				obj_id += "Basin"
			if obj_id in self.small or obj_id in self.large:
				gt_objs.append(obj_id)
		for obj_name in self.agent.last_event.instance_detections2D:
			if self.cp_goal in obj_name:
				xyxy = self.agent.last_event.instance_detections2D[obj_name]
		...
		for i in range(len(yolo_result.boxes.cls)):
			cls_id = yolo_result.boxes.cls[i]
			name = yolo_result.names[int(cls_id)]
			yolo_objs.append(name)
		for gt_obj in gt_objs:
			if gt_obj not in yolo_objs:
				yolo_have = False
				loss_obj = gt_obj
				break
		if yolo_have:
		...
		imgs_path = '/raid/failed_seen/images'
		masks_path = '/raid/failed_seen/masks'
		meta_path = '/raid/failed_seen/meta'
		this_name = f"{self.agent.scene_num}_yolo_seg-yolo_have_{yolo_have}-gt_have_{gt_have}-loss_{loss_obj}-step_{self.agent.steps_taken}_{self.img_tag}_{gt_size}"
		cv2.imwrite(os.path.join(imgs_path, this_name+'.png'), self.agent.event.frame[:, :, ::-1])
		cv2.imwrite(os.path.join(masks_path, this_name+'.png'), self.agent.event.instance_segmentation_frame)
		color_to_obj_id_type = {}
		boxes = self.agent.last_event.instance_detections2D
		# print(boxes)
		# frame = env.last_event.cv2img
		# frame = cv2.resize(frame, (320, 320))
		# cv2.imshow("frame", frame)
		# cv2.waitKey(0)
		for color, object_id in self.agent.last_event.color_to_object_id.items():
			if object_id not in boxes:
				continue
			color_to_obj_id_type[str(color)] = object_id
		with open(os.path.join(meta_path, this_name+'.json'), 'w') as f:
	...
	def get_instance_mask_seg_alfworld_both(self):
		...
		ims = [rgb]
		results_small = {
			'boxes': torch.empty(1, 4, device=self.sem_seg_gpu),
			'scores': torch.empty(1, device=self.sem_seg_gpu),
			'masks': torch.empty(1, 1, 300, 300, device=self.sem_seg_gpu),
			'labels': torch.empty(1, device=self.sem_seg_gpu)
		}     
		results_large = {
			'boxes': torch.empty(1, 4, device=self.sem_seg_gpu),
			'scores': torch.empty(1, device=self.sem_seg_gpu),
		...
		for i in range(len(result.boxes.cls)):
			cls_id = result.boxes.cls[i]
			name = result.names[int(cls_id)]
			resized_mask = cv2.resize(result.masks.data[i].cpu().numpy(), (300, 300)[::-1])
			resized_mask = torch.tensor(resized_mask, dtype=torch.float32, device=self.sem_seg_gpu).unsqueeze(0)
			boxes = result.boxes.xyxy[i]*(300/320)
			# print(f"{name} is at {keypoint}")
			if name in self.small:
				results_small['boxes'] = torch.cat([boxes.unsqueeze(0), results_small['boxes']], dim=0)
				results_small["scores"] = torch.cat([result.boxes.conf[i].unsqueeze(0), results_small["scores"]], dim=0)
				results_small["masks"] = torch.cat([resized_mask.unsqueeze(0), results_small["masks"]], dim=0)
				results_small["labels"] = torch.cat([torch.tensor(self.small.index(name)).cuda(self.sem_seg_gpu).unsqueeze(0), results_small["labels"]], dim=0)
			elif name in self.large:
				results_large['boxes'] = torch.cat([results_large['boxes'], boxes.unsqueeze(0)], dim=0)
				results_large["scores"] = torch.cat([results_large["scores"], result.boxes.conf[i].unsqueeze(0)], dim=0)
				results_large["masks"] = torch.cat([results_large["masks"], resized_mask.unsqueeze(0)], dim=0)
				results_large["labels"] = torch.cat([results_large["labels"], torch.tensor(self.large.index(name)).cuda(self.sem_seg_gpu).unsqueeze(0)], dim=0)

		# seg_frame = result.plot()
		# # seg_frame = cv2.resize(seg_frame, (512, 512))
		# # seg_frame = cv2.cvtColor(seg_frame, cv2.COLOR_RGB2BGR)
		# cv2.imshow("obj", seg_frame)
		# cv2.waitKey(1) 
		...
		# 	# seg_frame = cv2.resize(seg_frame, (512, 512))
		# 	# seg_frame = cv2.cvtColor(seg_frame, cv2.COLOR_RGB2BGR)
		# 	cv2.imwrite(self.agent.picture_folder_name + "Sem/" + "Sem_" + str(self.agent.steps_taken) + ".png", seg_frame)  
		# ******************
		...
		show_all = hasattr(self.args, "override_sem_seg_thresh")
		for k in range(len(results_small['labels'])):
			# confident = results_small['scores'][k] > self.args.sem_seg_threshold_small
			confident = True
			is_search_target = results_small['labels'][k].item() in desired_classes_small

			if confident and (show_all or is_search_target):
				indices_small.append(k)

		for k in range(len(results_large['labels'])):
			# confident = results_large['scores'][k] > self.args.sem_seg_threshold_large
			confident = True
			is_search_target = results_large['labels'][k].item() in desired_classes_large
	...
	def segmentation_ground_truth(self):
		...
		# semantic_seg = np.zeros((self.args.env_frame_height, self.args.env_frame_width, self.args.num_sem_categories))
		semantic_seg = np.zeros((512, 512, self.args.num_sem_categories))
		...
		if self.args.ignore_categories:
			...
			for k, v in instance_mask.items():
				...
				if success ==1:
					...
					pass
		semantic_seg = cv2.resize(semantic_seg, (300, 300))
	...
	def get_instance_mask_from_obj_type(self, object_name, target_coord=None):
		scores, masks = [-10], [np.zeros((512, 512))]
		...
		mask = masks[np.argmax(scores)].astype(np.float)
		if np.sum(mask) == 0:
			mask = None
		else:
	...
	def sem_seg_get_instance_mask_from_obj_type(self, object_name, target_coord=None, ignore_sliced=False):
		max_score = -10  # this value thresholds the lowest allowable value for the case in which target_coord is not None

		if ignore_sliced:
			object_name = object_name.replace("Sliced", '')

		if object_name in self.agent.cat_equate_dict:
			object_name = self.agent.cat_equate_dict[object_name]

		if object_name in self.large_objects2idx:
			name, idx2obj = "large", self.large_idx2large_object
		else:
			name, idx2obj = "small", self.small_idx2small_object

		# search for the most appropriate mask from the list of predicted instance segmentations
		mask = np.zeros((300, 300))
		for i in range(len(self.segmented_dict[name]['scores'])):
			category = idx2obj[self.segmented_dict[name]['classes'][i].item()]
			if category == object_name:
				v = self.segmented_dict[name]['masks'][i]
				score = self.segmented_dict[name]['scores'][i] if (target_coord is None) else self.maskDistance(v, target_coord)
...
class HelperFunctions:
    # Changed by Trisoil
	def diff_two_frames(f1, f2, pre_depth, now_depth):
		...
		for lab in unique_labels:
			...
			lab_area[lab] = np.sum(len(wheres[0]))
			if lab_area[lab] > 100:
				average_now_depth = np.average(now_depth[wheres])
				average_pre_depth = np.average(pre_depth[wheres])
				if average_now_depth < average_pre_depth and average_pre_depth - average_now_depth > max_delta_depth:
					max_delta_depth = average_pre_depth - average_now_depth
					smallest_lab = lab
	...
	def diff_two_frames_max(f1, f2, pre_depth, now_depth):
		diff1 = np.where(f1[:, :, 0] != f2[:, :, 0])
		diff2 = np.where(f1[:, :, 1] != f2[:, :, 1])
		diff3 = np.where(f1[:, :, 2] != f2[:, :, 2])
		diff_mask = np.zeros((300,300)); return_mask = np.zeros((300,300))
