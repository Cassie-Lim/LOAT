...
class SemgnetationHelper:
	...
	def __init__(self, agent):
		...
		if args.use_sem_seg:
			...
			self.yolo_model = YOLO("/home/tmn/Desktop/cyw/prompter-alfred-YOLO-no_replan_v2/best.pt").to(self.sem_seg_gpu)
			# 最好的模型保存在v2那个版本那里
	...
	def get_instance_mask_seg_alfworld_both(self):
		...
		result = self.yolo_model.predict(ims, conf=0.3, imgsz=640, verbose=False)[0]

		results_large['boxes'] = results_large['boxes'].tolist()
		results_large['scores'] = results_large['scores'].tolist()
		results_large['masks'] = results_large['masks'].tolist()
		results_large['labels'] = results_large['labels'].tolist()
		results_small['boxes'] = results_small['boxes'].tolist()
		results_small['scores'] = results_small['scores'].tolist()
		results_small['masks'] = results_small['masks'].tolist()
		results_small['labels'] = results_small['labels'].tolist()
  
		for i in range(len(results_small['masks'])):
			results_small['masks'][i] = np.array(results_small['masks'][i])

		for i in range(len(result.boxes.cls)):
			cls_id = result.boxes.cls[i]
			name = result.names[int(cls_id)]
			resized_mask = cv2.resize(np.array(result.masks.data[i].tolist()), (300, 300)[::-1])
			boxes = [x*0.46875 for x in result.boxes.xyxy[i].tolist()]
			# print(f"{name} is at {keypoint}")
			# if name in self.small and self.small.index(name) not in results_small['labels']:
			# 	results_small['boxes'].append(boxes)
			# 	results_small["scores"].append(result.boxes.conf[i].tolist())
			# 	results_small["masks"].append([resized_mask.tolist()])
			# 	results_small["labels"].append(torch.tensor(self.small.index(name)).tolist())
			# elif name in self.large and self.large.index(name) not in results_large['labels']:
			# 	results_large['boxes'].append(boxes)
			# 	results_large["scores"].append(result.boxes.conf[i].tolist())
			# 	results_large["masks"].append([resized_mask.tolist()])
			# 	results_large["labels"].append(torch.tensor(self.large.index(name)).tolist())

		# if len(results_large['masks']) == 0:
		# 	results_large['masks'].append([])
		# if len(results_small['masks']) == 0:
		# 	results_small['masks'].append([])

		results_small['boxes'] = torch.as_tensor(results_small['boxes'], device="cuda")
		results_small['scores'] = torch.as_tensor(results_small['scores'], device="cuda")
		results_small['masks'] = torch.as_tensor(results_small['masks'], device="cuda")
		results_small['labels'] = torch.as_tensor(results_small['labels'], device="cuda")
		results_large['boxes'] = torch.as_tensor(results_large['boxes'], device="cuda")
		results_large['scores'] = torch.as_tensor(results_large['scores'], device="cuda")
		results_large['masks'] = torch.as_tensor(results_large['masks'], device="cuda")
		results_large['labels'] = torch.as_tensor(results_large['labels'], device="cuda")
	...
	def get_instance_mask_from_obj_type(self, object_name, target_coord=None):
		scores, masks = [-10], [np.zeros((300, 300))]
