# 这一整个文件已经被替换，原本应该在envs文件夹下面
# 跑replan的时候需要
import numpy as np·
import torch

# ***********mod by me*****************
from trail.yolo_mask_rcnn_small.sem_exp_thor import Sem_Exp_Env_Agent_Thor
# from add_bycyw.code.sem_exp_thor import Sem_Exp_Env_Agent_Thor
# ***********************
...
# *******************************
# 将原本读取from_id,to_id的所有内容，改为读取指定列表中的内容
def construct_envs_alfred(args):
    args_list = []
    scene_names_list = [[] for i in range(args.num_processes)]
    ...
    # # # **********
    # # # for test 递归超出限制
    # scene_names_list[1] += scene_names_list[0][1:]
    # scene_names_list[0] = [scene_names_list[0][0]]
    # # # *********
    del files
    for _ in range(args.num_processes):
        args_list.append(args)
