...
from models.segmentation.alfworld_mrcnn import load_pretrained_model
...
class SemgnetationHelper:
	...
	def __init__(self, agent):
		...
		if args.use_sem_seg:
			...
			# self.yolo_model = YOLO("/home/tmn/work/prompter-alfred-YOLO-no_replan/models/best.pt").to(self.sem_seg_gpu)
			self.yolo_model = YOLO("/home/tmn/Desktop/cyw/prompter-alfred-YOLO-no_replan_v2/best.pt").to(self.sem_seg_gpu)

			print("the segmentation model is yolo samll + mask rcnn")
	...
	def get_instance_mask_seg_alfworld_both(self):
		...
		result = self.yolo_model(self.agent.event.cv2img, conf=0.3, imgsz=512, verbose=False, device=self.sem_seg_gpu)[0]	
		...
		# # seg_frame = cv2.resize(seg_frame, (1024, 1024))
